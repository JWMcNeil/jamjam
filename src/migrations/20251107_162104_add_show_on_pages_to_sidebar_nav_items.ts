...
export async function down({ db, payload }: MigrateDownArgs): Promise<void> {
  // Drop indexes first
  try {
    await db.run(sql`DROP INDEX IF EXISTS \`sidebar_nav_items_show_on_pages_order_idx\``)
    await db.run(sql`DROP INDEX IF EXISTS \`sidebar_nav_items_show_on_pages_parent_id_idx\``)
  } catch (error: unknown) {
    const errorMessage = error instanceof Error ? error.message : String(error)
  ...
  // Drop the table
  try {
    await db.run(sql`DROP TABLE IF EXISTS \`sidebar_nav_items_show_on_pages\``)
