...
type ImageMasonryGridBlockProps = {
  gap?: 'small' | 'medium' | 'large' | null
  images?: ImageMasonryGridItem[] | null
}
...
export const ImageMasonryGrid: React.FC<Props> = (props) => {
  ...
  const preparedImages = useMemo(() => {
    ...
    const filtered = images.filter((item: ImageMasonryGridItem) => {
  ...
  return (
    ...
    >
      <div className={cn(columnClasses, gapClasses[gapValue as keyof typeof gapClasses])}>
        {preparedImages.map((photo: (typeof preparedImages)[number]) => {
